...
public class ActionData {
	...
	public static ActionData createAlert(String beneficiaryType, String scheduleName, String visitCode,
		...
		return new ActionData(TARGET, TYPE).with(BENEFICIARY_TYPE, beneficiaryType).with(SCHEDULE_NAME, scheduleName)
		        .with(VISIT_CODE, visitCode).with(ALERT_STATUS, alertStatus.value())
		        .with(START_DATE, startDate.toLocalDate().toString()).with(EXPIRY_DATE, expiryDate.toLocalDate().toString());
	}
	
	public static ActionData markAlertAsClosed(String visitCode, String completionDate) {
		return new ActionData(TARGET, "closeAlert").with(VISIT_CODE, visitCode).with(COMPLETION_DATE, completionDate);
	}
	
	public static ActionData reportForIndicator(String indicator, String annualTarget, String monthSummaries) {
		return new ActionData("report", indicator).with(ANNUAL_TARGET, annualTarget).with(MONTHLY_SUMMARIES, monthSummaries);
	}
	
	public static ActionData closeBeneficiary(String target, String reasonForClose) {
		return new ActionData(target, "close").with(REASON_FOR_CLOSE, reasonForClose);
	}
